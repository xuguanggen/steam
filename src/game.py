...
def get_usergames_page(htmlPath):
    ...
    for game in src_data:
        ...
        hours_forever = str(game['hours_forever']).replace(',','') if "hours_forever" in game.keys() else "-1"
        #print('\t'.join([appid, name, hours, hours_forever]))
        this_game_info = ",".join([appid, hours_forever])
...
if __name__=='__main__':
    ...
    out_file = sys.argv[3]
    all_infos = get_pages(url, game_html)
